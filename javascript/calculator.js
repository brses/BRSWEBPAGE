...
function CalcTerm() {
    ...
    if (checkLoan(document.getElementById('repay').LoanAmt2) &&
        ...
        if (RepayAmount < MinRepay) {
            Msg = "Sorry, the minimum repayment amount must be " + MinRepay + " Rupees."
            alert(Msg)
...
function showMyCalculator(show, price) {
    var params;
    if (price == 0) {
        params = "show=" + show;
        generalCalculatorAjax('resource_content', base_url + "/home_loans/start_calc", params);
    } else {
        params = "show=" + show + "&price=" + price;
        generalCalculatorAjax('resource_content', base_url + "/home_loans/start_calc", params);
    }
}

function generalCalculatorAjax(id, url, params) {
